...
void InitializeData(std::vector<float> &inputX, std::vector<float> &inputY, std::vector<float> &inputZ,
 std::vector<float> &golden, std::vector<int64_t> &shape)
{
...
int main(int argc, char **argv)
{
    ...
    std::vector<float> inputXHostData(shape[0] * shape[1]);
    std::vector<float> inputYHostData(shape[0] * shape[1]);
    std::vector<float> outputZHostData(shape[0] * shape[1]);
    std::vector<float> goldenData(shape[0] * shape[1]); 

    InitializeData(inputXHostData, inputYHostData, outputZHostData, goldenData, shape);
    ...
    ret = CreateAclTensor(inputXHostData, shape, &inputXDeviceAddr, aclDataType::ACL_FLOAT, &inputX);
    CHECK_RET(ret == ACL_SUCCESS, DestroyResources(tensors, deviceAddrs, stream, deviceId); return 1);
    // Create inputY aclTensor
    ret = CreateAclTensor(inputYHostData, shape, &inputYDeviceAddr, aclDataType::ACL_FLOAT, &inputY);
    CHECK_RET(ret == ACL_SUCCESS, DestroyResources(tensors, deviceAddrs, stream, deviceId); return 1);
    // Create outputZ aclTensor
    ret = CreateAclTensor(outputZHostData, shape, &outputZDeviceAddr, aclDataType::ACL_FLOAT, &outputZ);
    CHECK_RET(ret == ACL_SUCCESS, DestroyResources(tensors, deviceAddrs, stream, deviceId); return 1);
