...
## 概述
本样例基于AddCustom算子工程，介绍了基于ATVC的自定义算子工程调用。

## 目录结构介绍
```
├── add
│   ├── AclNNInvocationNaive   // 通过aclnn调用的方式调用AddCustom算子
...
### 2. 生成自定义算子工程，复制host和kernel实现并编译算子<a name="operatorcompile"></a>
  ...
  - 切换到msOpGen脚本install.sh所在目录
    ...
    cd atvc/examples/ops_aclnn/add
    ```
...
### 4. 调用执行算子工程
- [aclnn调用AddCustom算子工程](./AclNNInvocationNaive/README.md)
