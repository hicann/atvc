...
get_package_install_type() {
    ...
    _install_type_gpit="$(grep -i "ATVC_install_type=" "${_install_info}" | cut -d"=" -f2-)"
...
get_package_feature_type() {
    ...
    if [ "${_package}" = "opp" ]; then
        ...
        _feature_type_gpft="$(grep -i "ATVC_feature_type=" "${_install_info}" | cut -d"=" -f2-)"
    fi
...
get_package_chip_type() {
    ...
    if [ "${_package}" = "opp" ]; then
        _cihp_gpct="$(grep -i "ATVC_chip_type=" "${_install_info}" | cut -d"=" -f2-)"
    else
