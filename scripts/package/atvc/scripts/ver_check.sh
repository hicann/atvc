...
check_atvc_version_file() {
  if [ -f "${_CURR_PATH}/../../version.info" ]; then
    ver_info="${_CURR_PATH}/../../version.info"
  # atvc/version.info -> atvc
  elif [ -f "${_DEFAULT_INSTALL_PATH}/atvc/version.info" ]; then
    ver_info="${_DEFAULT_INSTALL_PATH}/atvc/version.info"
  else
    logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST}; The [atvc] version.info not exists."
  fi
  return
}
...
show_relation() {
  ...
  if [ "$relation_situation" = "SUCC" ]; then
    logandprint "[INFO]: Relationship of atvc with ${req_pkg_name_val} in path ${req_pkg_path} check successfully"
    return 0
  else
    logandprint "[WARNING]: Relationship of atvc with ${req_pkg_name_val} in path ${req_pkg_path} check failed. \
...
version_check() {
  ...
  #get atvc version
  check_atvc_version_file
  ret_check_atvc_version_file=$ver_info
  #get checked path
  ...
  if [ "${ret_check_path_pre}" != "" ]; then
    for var in ${ret_check_path_pre}; do
      ...
      #check relation
      check_relation "${ret_check_atvc_version_file}" "${ret_last_component}" "${ret_check_version_file}"
