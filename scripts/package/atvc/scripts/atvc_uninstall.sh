...
TARGET_MOULDE_DIR=${TARGET_VERSION_DIR}/${ATVC_PLATFORM_DIR} # TARGET_INSTALL_PATH + PKG_VERSION_DIR + ATVC_PLATFORM_DIR
...
check_installed_type() {
  ...
  if [ "${type}" != "run" ] &&
    ...
    [ "${type}" != "upgrade" ] &&
    [ "${type}" != "devel" ]; then
    logandprint "[ERROR]: ERR_NO:${UNAME_NOT_EXIST};ERR_DES:Install type of atvc is not right!"
    exit 1
  fi
}
...
remove_module() {
  ...
  logandprint "[INFO]: Delete the installed atvc source files in (${TARGET_VERSION_DIR})."

  bash "${COMMON_PARSER_FILE}" --package="${ATVC_PLATFORM_DIR}" --uninstall --recreate-softlink \
  ...
  log_with_errorlevel "$?" "error" "[ERROR]: ERR_NO:${OPERATE_FAILED};ERR_DES:Uninstall atvc failed."
...
remove_atvc() {
  ...
  if [ "${UNINSTALL_MODE}" != "upgrade" ]; then
    ...
    log_with_errorlevel "$?" "warn" "[WARNING] Delete atvc install info file failed, please delete it by yourself."
  fi
...
#   [ -d ${arch_include_dir}/aclnn_kernels ] && rm -rf "${arch_include_dir}/aclnn_kernels"
  # remove all softlink
  find ${TARGET_INSTALL_PATH}/latest/ -type l -lname "*/${ATVC_PLATFORM_DIR}/*" -delete
}

logandprint "[INFO]: Begin uninstall the atvc."
...
main() {
  ...
  remove_dir_if_empty ${TARGET_MOULDE_DIR}
  ...
  logandprint "[INFO]: atvc package uninstalled successfully! Uninstallation takes effect immediately."
}
