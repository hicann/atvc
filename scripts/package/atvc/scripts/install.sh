...
ATVC_PLATFORM_DIR=atvc
ATVC_PLATFORM_UPPER=$(echo "${ATVC_PLATFORM_DIR}" | tr '[:lower:]' '[:upper:]')
...
INSTALL_SHELL_FILE="${CURR_PATH}/atvc_install.sh"
...
ATVC_COMMON_FILE="${CURR_PATH}/atvc_common.sh"
...
TARGET_MOULDE_DIR=""  # TARGET_INSTALL_PATH + PKG_VERSION_DIR + ATVC_PLATFORM_DIR
...
KEY_INSTALLED_TYPE="${ATVC_PLATFORM_UPPER}_INSTALL_TYPE"
KEY_INSTALLED_PATH="${ATVC_PLATFORM_UPPER}_INSTALL_PATH_VAL"
KEY_INSTALLED_VERSION="${ATVC_PLATFORM_UPPER}_VERSION"
KEY_INSTALLED_FEATURE="${ATVC_PLATFORM_UPPER}_INSTALL_FEATURE"
KEY_INSTALLED_CHIP="${ATVC_PLATFORM_UPPER}_INSTALL_CHIP"
...
startlog() {
  echo "[ATVC] [$(getdate)] [INFO]: Start Time: $(getdate)"
}

exitlog() {
  echo "[ATVC] [$(getdate)] [INFO]: End Time: $(getdate)"
}
...
clean_before_reinstall() {
  ...
  if [ -z "${existed_files}" ]; then
    logandprint "[INFO]: Directory is empty, directly install atvc."
    return 0
  fi

  if [ "${IS_QUIET}" = "y" ]; then
    logandprint "[WARNING]: Directory has file existed or installed atvc,\
 are you sure to keep installing atvc in it? y"
  else
    if [ ! -f "${TARGET_MOULDE_DIR}/ascend_install.info" ]; then
      logandprint "[INFO]: Directory has file existed, do you want to continue? [y/n]"
    else
      logandprint "[INFO]: atvc package has been installed on the path $(get_installed_info "${KEY_INSTALLED_PATH}"),\
 ...
 and the version of this package is ${RUN_PKG_VERSION}, do you want to continue? [y/n]"
    ...
    while true; do
      read yn
      if [ "$yn" = "n" ]; then
        logandprint "[INFO]: Exit to install atvc."
        exitlog
        exit 0
      elif [ "$yn" = "y" ]; then
        break
  ...
  if [ "${installed_path}" = "${TARGET_VERSION_DIR}" ]; then
    logandprint "[INFO]: Clean the installed atvc before install."
 ...
 path or clean the previous version atvc install info (${INSTALL_INFO_FILE}) and then reinstall it."
      return 1
    fi
...
check_atvc_version_info() {
  if [ -f "${CURR_PATH}/../../version.info" ]; then
    atvc_ver_info="${CURR_PATH}/../../version.info"
  elif [ -f "${DEFAULT_INSTALL_PATH}/${ATVC_PLATFORM_DIR}/version.info" ]; then
    atvc_ver_info="${DEFAULT_INSTALL_PATH}/${ATVC_PLATFORM_DIR}/version.info"
  else
    logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST}; The [${ATVC_PLATFORM_DIR}] version.info not exists."
    exitlog
    exit 1
  fi
  return
...
check_relation() {
  ...
  if [ -f "${COMMON_INC_FILE}" ]; then
    . "${COMMON_INC_FILE}"
    check_pkg_ver_deps "${atvc_ver_info_val}" "${req_pkg_name}" "${req_pkg_version}"
...
show_relation() {
  ...
  if [ "$relation_situation" = "SUCC" ]; then
    logandprint "[INFO]: Relationship of atvc with ${req_pkg_name_val} in path ${req_pkg_path} checked successfully"
  else
    logandprint "[WARNING]: Relationship of atvc with ${req_pkg_name_val} in path ${req_pkg_path} checked failed."
  fi
  return
}
...
find_version_check() {
  ...
  check_atvc_version_info
  ret_check_atvc_version_info=$atvc_ver_info
  for var in ${ccec_compiler_path}; do
    ...
    #check relation
    check_relation "${ret_check_atvc_version_info}" "${ret_pkg_name}" "${ret_check_version_file}"
...
path_version_check() {
  path_env_list="$1"
  check_atvc_version_info
  ret_check_atvc_version_info_name=$atvc_ver_info
  ...
  for var in ${array}; do
    ...
    if [ "${path_ccec_compile}" != "" ]; then
      ...
      #check relation
      check_relation "${ret_check_atvc_version_info_name}" "${ret_pkg_name}" "${ret_check_version_file_val}"
...
check_docker_path() {
  docker_path="$1"
  if [ "${docker_path}" != "/"* ]; then
    echo "[ATVC] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Parameter --docker-root\
 must with absolute path that which is start with root directory /. Such as --docker-root=/${docker_path}"
    exitlog
    exit 1
  fi
  if [ ! -d "${docker_path}" ]; then
    echo "[ATVC] [ERROR]: ERR_NO:${FILE_NOT_EXIST}; The directory:${docker_path} not exist, please create this directory."
    exitlog
    exit 1
  fi
}
...
judgment_path() {
  ...
  if [ $? -ne 0 ]; then
    echo "[ATVC][ERROR]: The atvc install path ${1} is invalid, only characters in [a-z,A-Z,0-9,-,_] are supported!"
    exitlog
    exit 1
  fi
}
...
check_install_path() {
  ...
  if [ "x${TARGET_INSTALL_PATH}" = "x" ]; then
    echo "[ATVC] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Parameter --install-path\
 ...
 not support that the install path contains space character."
  ...
  if [ "x${prefix}" = "x" ]; then
    ...
    if [ x"${prefix}" = "x" ]; then
      ...
      echo "[ATVC] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES: Run package path is invalid: $RUN_PATH"
      exitlog
      exit 1
    fi
  fi
...
interact_pre_check() {
  ...
  if [ "$?" != 0 ]; then
    if [ "${IS_QUIET}" = y ]; then
      logandprint "[WARNING]: Precheck of atvc execute failed! do you want to continue install? y"
    else
      logandprint "[WARNING]: Precheck of atvc execute failed! do you want to continue install?  [y/n] "
      while true; do
        read yn
        if [ "$yn" = "n" ]; then
          echo "stop install atvc!"
          exit 1
        elif [ "$yn" = y ]; then
          break
        else
...
#atvc_install_path_curr=`echo "$2" | cut -d"/" -f2- `
  ...
  while true; do
    ...
    case "$1" in
      ...
      --install-for-all)
        ...
        ;;
      -*)
        echo "[ATVC] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Unsupported parameters [$1],\
...
check_opts() {
  ...
  if [ "${CONFLICT_CMD_NUMS}" != 1 ]; then
    echo "[ATVC] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:\
...
init_env() {
  get_install_package_dir "TARGET_MOULDE_DIR" "${VERSION_INFO_FILE}" "${TARGET_INSTALL_PATH}" "${ATVC_PLATFORM_DIR}"
  ...
  UNINSTALL_SHELL_FILE="${TARGET_MOULDE_DIR}/script/atvc_uninstall.sh"
  ...
  logandprint "[INFO]: Execute the atvc run package."
  ...
  if [ "${installed_version}" = "" ]; then
    logandprint "[INFO]: Version of installing atvc is ${RUN_PKG_VERSION}."
  else
    if [ "${RUN_PKG_VERSION}" != "" ]; then
      logandprint "[INFO]: Existed atvc version is ${installed_version},\
 the new atvc version is ${RUN_PKG_VERSION}."
    fi
  fi
}
...
check_pre_install() {
 ...
 Please make sure that the atvc installed in (${VERCHECK_FILE}) and then set the correct install path."
    fi
...
install_package() {
  ...
  # precheck before install atvc module
  ...
  clean_before_reinstall
  if [ "$?" != 0 ]; then
    comm_log_operation "Install" "${IN_INSTALL_TYPE}" "ATVC" "$?" "${CMD_LIST}"
  fi
...
uninstall_package() {
 ...
 (${UNINSTALL_SHELL_FILE}) not exists. Please make sure that the atvc module\
 installed in (${TARGET_VERSION_DIR}) and then set the correct install path."
    ...
    comm_log_operation "Uninstall" "${IN_INSTALL_TYPE}" "ATVC" "$?" "${CMD_LIST}"
    exit 0
  fi
