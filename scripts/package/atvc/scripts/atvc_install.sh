...
init_install_env() {
  ...
  TARGET_MOULDE_DIR=${TARGET_VERSION_DIR}/${ATVC_PLATFORM_DIR}
...
install_atvc() {
  logandprint "[INFO]: Begin install atvc."
  local version_mod=""
  local module_mod=""
  ...
  logandprint "[INFO]: Update the atvc install info."

  update_install_infos "${TARGET_USERNAME}" "${TARGET_USERGROUP}" "${INSTALL_TYPE}" "${relative_path_val}"
  log_with_errorlevel "$?" "error" "[ERROR]: ERR_NO:${INSTALL_FAILED};ERR_DES:Update atvc install info failed."

  bash "${COMMON_PARSER_FILE}" --package="${ATVC_PLATFORM_DIR}" --install --username="${TARGET_USERNAME}" \
  ...
  log_with_errorlevel "$?" "error" "[ERROR]: ERR_NO:${INSTALL_FAILED};ERR_DES:Install atvc files failed."
...
main() {
  logandprint "[INFO]: Command atvc_install"

  get_opts "$@"

  init_install_env

  get_package_upgrade_version_dir "upgrade_version_dir" "$TARGET_INSTALL_PATH" "${ATVC_PLATFORM_DIR}"
  get_package_last_installed_version "last_installed" "$TARGET_INSTALL_PATH" "${ATVC_PLATFORM_DIR}"
  ...
  log_with_errorlevel "$?" "error" "[ERROR]: ERR_NO:${INSTALL_FAILED};ERR_DES:Change atvc onwership failed.."
  ...
  logandprint "[INFO]: atvc package installed successfully! The new version takes effect immediately."
}
