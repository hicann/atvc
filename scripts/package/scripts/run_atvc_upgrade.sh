#!/bin/sh
# Perform upgrade for atvc package
# -----------------------------------------------------------------------------------------------------------
...
install_info="${common_parse_dir}/share/info/atvc/ascend_install.info"
if [ -f "$install_info" ]; then
    chip_type=$(get_install_param "Atvc_Chip_Type" "${install_info}")
    feature_type=$(get_install_param "Atvc_Feature_Type" "${install_info}")
fi
...
output_progress() {
    new_echo "INFO" "atvc upgrade upgradePercentage:$1%"
    log "INFO" "atvc upgrade upgradePercentage:$1%"
}
...
##########################################################################
log "INFO" "step into run_atvc_upgrade.sh ......"
...
new_upgrade() {
    if [ ! -d "${sourcedir}" ]; then
        log "INFO" "no need to upgrade atvc files."
        return 0
    fi
    output_progress 10
    ...
    sh "$curpath/install_common_parser.sh" --package="atvc" --install --username="$username" --usergroup="$usergroup" --set-cann-uninstall --upgrade \
