...
SOURCE_INSTALL_COMMON_PARSER_FILE="${common_parse_dir}/share/info/atvc/script/install_common_parser.sh"
SOURCE_FILELIST_FILE="${common_parse_dir}/share/info/atvc/script/filelist.csv"
...
##########################################################################
log "INFO" "step into run_atvc_uninstall.sh ......"
log "INFO" "uninstall target dir $common_parse_dir, type $common_parse_type."

if [ ! -d "$common_parse_dir/share/info/atvc" ]; then
    log "ERROR" "ERR_NO:0x0001;ERR_DES:path $common_parse_dir/share/info/atvc is not exist."
    exit 1
fi

new_uninstall() {
    if [ ! -d "${common_parse_dir}/share/info/atvc" ]; then
        log "INFO" "no need to uninstall atvc files."
        return 0
    fi
    ...
    if [ "$pkg_is_multi_version" = "true" ] && [ "$hetero_arch" = "y" ]; then
        ...
        if [ -e "$package_db_info" ]; then
            ...
            local cann_path="$(realpath $linux_path/../..)/cann"
            local pkgs="$(cut -d'|' -f2 $package_db_info | sort -u)"
            if [ "$pkgs" = "atvc" ]; then
                if [ -L "$cann_path/$arch_path" ] && [ "$(realpath $linux_path)" = "$(realpath $cann_path/$arch_path)" ]; then
                    rm -f "$cann_path/$arch_path"
                fi
            elif [ -n "$pkgs" ] && [ -d "$cann_path" ]; then
                if [ ! -e "$cann_path/$arch_path" ] || [ -L "$cann_path/$arch_path" ]; then
                    ln -srfn "$linux_path" "$cann_path"
                fi
            fi
        fi
    fi
    ...
    sh "$SOURCE_INSTALL_COMMON_PARSER_FILE" --package="atvc" --uninstall --username="$username" --usergroup="$usergroup" ${recreate_softlink_option} \
    ...
    if [ -n "$cann_path" ] && [ -d "$cann_path" ] && [ "x$(ls -A $cann_path 2>&1)" = "x" ]; then
        rm -rf "$cann_path"
    fi
