#!/bin/sh
# Perform custom_install script for atvc package
# -----------------------------------------------------------------------------------------------------------
...
log() {
    ...
    if [ "$log_type" = "INFO" -o "$log_type" = "WARNING" -o "$log_type" = "ERROR" ]; then
        echo -e "[Atvc] [$cur_date] [$log_type]: $*"
    else
        echo "[Atvc] [$cur_date] [$log_type]: $*" 1> /dev/null
    fi
    echo "[Atvc] [$cur_date] [$log_type]: $*" >> "$logfile"
}
...
custom_install() {
    if [ -z "$common_parse_dir/share/info/atvc" ]; then
        log "ERROR" "ERR_NO:0x0001;ERR_DES:atvc directory is empty"
        exit 1
    fi
    return 0
}
