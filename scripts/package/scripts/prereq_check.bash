...
log() {
    ...
    if [ "$log_type" = "INFO" ]; then
        $echo_cmd "[Atvc] [$cur_date] [$log_type]: \033[32m$*\033[0m"
    elif [ "$log_type" = "WARNING" ]; then
        $echo_cmd "[Atvc] [$cur_date] [$log_type]: \033[33m$*\033[0m"
    elif [ "$log_type" = "ERROR" ]; then
        $echo_cmd "[Atvc] [$cur_date] [$log_type]: \033[31m$*\033[0m"
    else
        $echo_cmd "[Atvc] [$cur_date] [$log_type]: $*"
    fi
}
