#!/bin/sh
# Perform install/upgrade/uninstall for atvc package
# -----------------------------------------------------------------------------------------------------------
...
atvc_func_path="${curpath}/atvc_func.sh"
...
log() {
    ...
    local log_type="$1"
    local log_msg="$2"
    local log_format="[Atvc] [$cur_date] [$log_type]: $log_msg"
...
update_install_param() {
    ...
    local install_info_key_array="Atvc_Install_Type Atvc_Chip_Type Atvc_Feature_Type Atvc_UserName Atvc_UserGroup Atvc_Install_Path_Param Atvc_Arch_Linux_Path Atvc_Hetero_Arch_Flag"
...
update_install_info_feature() {
    ...
    if [ "$featuremode" = "all" ] || [ "$operation" = "Upgrade" ]; then
        update_install_param "Atvc_Feature_Type" "$featuremode" "$install_info"
        return
    fi
    local current_featuremode=$(get_install_param "Atvc_Feature_Type" "$install_info")
...
update_install_info() {
    ...
    update_install_param "Atvc_Install_Type" "$installmode" "$install_info"
    update_install_param "Atvc_Chip_Type" "$chipmode" "$install_info"
    update_install_info_feature
    update_install_param "Atvc_UserName" "$username" "$install_info"
    update_install_param "Atvc_UserGroup" "$usergroup" "$install_info"
    update_install_param "Atvc_Install_Path_Param" "$install_path_param" "$install_info"
    update_install_param "Atvc_Arch_Linux_Path" "$arch_linux_path" "$install_info"
    update_install_param "Atvc_Hetero_Arch_Flag" "$hetero_arch" "$install_info"
}
...
install_run() {
    ...
    local atvc_install_path_param=""
    update_install_path
    update_install_info
    local atvc_input_install_path=$(get_install_param "Atvc_Install_Path_Param" "${install_info}")
    local atvc_install_type=$(get_install_param "Atvc_Install_Type" "${install_info}")
    if [ "$is_docker_install" = "y" ]; then
        atvc_install_path_param=$(concat_docker_install_path "${docker_root}" "${atvc_input_install_path}")
    else
        atvc_install_path_param="${atvc_input_install_path}"
    fi
    if [ "$1" = "install" ]; then
        unchattr_files
        chmod_start
        new_echo "INFO" "install ${atvc_install_path_param} ${atvc_install_type}"
        log "INFO" "install ${atvc_install_path_param} ${atvc_install_type}"
        sh "${curpath}/run_atvc_install.sh" "install" "${atvc_input_install_path}" "${atvc_install_type}" \
            "${is_quiet}" "${input_setenv}" "${docker_root}" "${in_install_for_all}"
        if [ $? -eq 0 ]; then
            update_version_info_version
            log "INFO" "Atvc package installed successfully! The new version takes effect immediately."
            log_operation "${operation}" "succeeded"
            chmod_end
        else
            chmod_end
            log "ERROR" "Atvc package install failed, please retry after uninstall!"
...
upgrade_run() {
    ...
    if [ -f "$install_info" ]; then
        local atvc_input_install_path=$(get_install_param "Atvc_Install_Path_Param" "${install_info}")
        local atvc_install_type=$(get_install_param "Atvc_Install_Type" "${install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i atvc_install_path_param "$install_info_old") -ne 0 ]; then
        local atvc_input_install_path="$(grep -iw atvc_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local atvc_install_type="$(grep -iw atvc_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ "$1" = "upgrade" ]; then
        chmod_start
        new_echo "INFO" "upgrade ${atvc_install_path_param} ${atvc_install_type}"
        log "INFO" "upgrade ${atvc_install_path_param} ${atvc_install_type}"
        sh "${curpath}/run_atvc_upgrade.sh" "upgrade" "${atvc_input_install_path}" "${atvc_install_type}" \
            "${is_quiet}" "${input_setenv}" "${docker_root}" "${in_install_for_all}"
        if [ $? -eq 0 ]; then
            update_version_info_version
            log "INFO" "Atvc package upgraded successfully! The new version takes effect immediately."
            log_operation "${operation}" "succeeded"
            chmod_end
        else
            chmod_end
            log "ERROR" "Atvc package upgrade failed, please retry after uninstall!"
            log_operation "${operation}" "failed!"
            exit_install_log 1
        fi
    fi
...
uninstall_run() {
    ...
    if [ -f "$upgrade_install_info" ]; then
        local atvc_input_install_path=$(get_install_param "Atvc_Install_Path_Param" "${upgrade_install_info}")
        local atvc_install_type=$(get_install_param "Atvc_Install_Type" "${upgrade_install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i atvc_install_path_param "$install_info_old") -ne 0 ]; then
        local atvc_input_install_path="$(grep -iw atvc_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local atvc_install_type="$(grep -iw atvc_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ ! -f "$upgrade_default_dir/script/run_atvc_uninstall.sh" ]; then
        log "WARNING" "run_atvc_uninstall.sh not found."
        return $?
    fi
    if [ "$1" = "uninstall" ]; then
        chmod_start "$upgrade_default_dir"
        new_echo "INFO" "uninstall ${atvc_install_path_param} ${atvc_install_type}"
        log "INFO" "uninstall ${atvc_install_path_param} ${atvc_install_type}"
        sh "$upgrade_default_dir/script/run_atvc_uninstall.sh" "uninstall" "${atvc_input_install_path}" "${atvc_install_type}" "${is_quiet}" \
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ $(id -u) -eq 0 ]; then
                if [ "$uninstall" = "y" ] && [ -f "$install_info_old" ] && [ $(grep -c -i atvc_install_path_param "$install_info_old") -ne 0 ]; then
                    sed -i '/atvc_install_path_param=/Id' "$install_info_old" 2> /dev/null
                    sed -i '/atvc_install_type=/Id' "$install_info_old" 2> /dev/null
                fi
            fi
            new_echo "INFO" "Atvc package uninstalled successfully! Uninstallation takes effect immediately."
            log "INFO" "Atvc package uninstalled successfully! Uninstallation takes effect immediately."
            log_operation "${operation}" "succeeded"
        else
            log "ERROR" "Atvc package uninstall failed!"
            log_operation "${operation}" "failed!"
            exit_uninstall_log 1
        fi
    fi
...
judgmentpath() {
    ...
    if [ $? -ne 0 ]; then
        log "ERROR" "The Atvc install_path ${1} is invalid, only characters in [a-z,A-Z,0-9,-,_] are supported!"
        exit 1
    fi
}
...
if [ "$featuremode" != "all" ]; then
    ...
    if [ "$ret" = "false" ]; then
        log "WARNING" "Atvc package doesn't contain features $featuremode, skip installation."
        exit 0
    fi
fi
...
if [ "$pkg_is_multi_version" = "true" ] && [ "$hetero_arch" != "y" ]; then
    default_dir="${pkg_install_path}/$pkg_version_dir/share/info/atvc"
else
    default_dir="${pkg_install_path}/share/info/atvc"
fi
...
if [ "$input_pre_check" = "y" ]; then
    ...
    pre_check
    if [ $? -ne 0 ]; then
        log "WARNING" "Atvc do pre check failed."
    else
...
if [ "$check" = "y" ]; then
    ...
    if [ -z "$pkg_version_dir" ]; then
        preinstall_check --install-path="$install_path_param" --script-dir="$curpath" --package="atvc" --logfile="$logfile" --docker-root="$docker_root"
        if [ $? -ne 0 ]; then
            exit_install_log 1
        else
    ...
    ver_check
    if [ -z "$pkg_version_dir" ]; then
        preinstall_process --install-path="$install_path_param" --script-dir="$curpath" --package="atvc" --logfile="$logfile" --docker-root="$docker_root"
        if [ $? -ne 0 ]; then
            exit_install_log 1
        else
            log "INFO" "version compatibility check successfully!"
...
uninstall_none_multi_version "$pkg_install_path/share/info/atvc"
check_install_for_all
create_default_install_dir_for_common_user
log_base_version
...
if [ "x$version_installed" != "x" -a "$version_installed" != "none" ] || [ -f "${install_info}" ]; then
    ...
    # 升级场景
        ...
        if [ -n "$pkg_version_dir" ]; then
            # if [ "$hetero_arch" = "y" ]; then
            #     export USE_SHARE_INFO="y"
            #     get_package_upgrade_install_info_hetero "upgrade_install_info"
            #     unset USE_SHARE_INFO
            # else
            #     export USE_SHARE_INFO="y"
            #     get_package_upgrade_install_info "upgrade_install_info" "$pkg_install_path" "atvc"
            #     unset USE_SHARE_INFO
            # fi
            upgrade_install_info=$install_info
    ...
    # 安装场景
        ...
        if [ "$is_quiet" = "n" ]; then
            log "INFO" "Atvc package has been installed on the path ${pkg_install_path}, the version is ${version_installed}, and the version of this package is ${version_of_package}, do you want to continue? [y/n]"
            while true
            do
                read yn
                if test "$yn" = n; then
