#!/bin/sh
# Perform install for atvc package
# -----------------------------------------------------------------------------------------------------------
...
output_progress() {
    new_echo "INFO" "atvc install upgradePercentage:$1%"
    log "INFO" "atvc install upgradePercentage:$1%"
}
...
##########################################################################
log "INFO" "step into run_atvc_install.sh ......"
...
new_install() {
    # if [ ! -d "${sourcedir}" ]; then
    #     log "INFO" "no need to install atvc files."
    #     return 0
    # fi
    output_progress 10
    ...
    sh "$curpath/install_common_parser.sh" --package="atvc" --install --username="$username" --usergroup="$usergroup" --set-cann-uninstall \
